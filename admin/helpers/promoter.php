...
class PromoterHelper
{
	 ...
	 */
	public static function addSubmenu($vName = 'pings')
	{
        
		JSubMenuHelper::addEntry(
			JText::_('Pings'),
			'index.php?option=com_promoter&view=pings',
			($vName == 'pings')
		);

		JSubMenuHelper::addEntry(
			JText::_('Projects'),
			'index.php?option=com_promoter&view=projects',
			($vName == 'projects')
		);

		JSubMenuHelper::addEntry(
			JText::_('Servers'),
			'index.php?option=com_promoter&view=servers',
			($vName == 'servers')
		);
	...
	public static function getExtensions() 
	{
		...
		$jv = new JVersion();
		$alt_libdir = ($jv->RELEASE < 1.6) ? JPATH_ADMINISTRATOR.DS.'components'.DS.'com_promoter' : null;
		JLoader::import('joomla.utilities.xmlelement', $alt_libdir);
		
		$xml = simplexml_load_file(JPATH_ADMINISTRATOR.DS.'components'.DS.'com_promoter'.DS.'elements'.DS.'extensions.xml', 'JXMLElement');		        
...
abstract class JHtmlPromoter
{
	...
	public static function extensions($ext) 
	{
		...
		foreach ($extensions as $extension) {   
			...
			$option->value = 'com_promoter.'.$extension->name;
			$options[] = $option;			
		}		
	...
	public static function categories($extension, $cat_id,$name="categories",$title="Select Category", $config = array('attributes'=>'class="inputbox"','filter.published' => array(0,1)))
	{
			...
			$query->from('#__promoter_categories AS a');
