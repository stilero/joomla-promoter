...
<form action="index.php?option=com_promoter&amp;view=projects" method="post" name="adminForm">
	<table>
		<tr>
...
<?php
  ...
  for ($i=0, $n=count( $this->items ); $i < $n; $i++):
    ...
    if (JRequest::getVar('function', null)) {
    	$onclick= "onclick=\"window.parent.jSelectProjects_id('".$row->id."', '".$this->escape($row->title)."', '','id')\" ";
    }  	
    
 	$link = JRoute::_( 'index.php?option=com_promoter&view=projects&task=edit&cid[]='. $row->id );
 	$row->id = $row->id; 	
  ...
  ?>
	<tr class="<?php echo "row$k"; ?>">
        ...
        <td><a class="modal" href="index.php?option=com_promoter&view=pinging&format=raw&project=<?php echo $row->id ?>" rel="{handler: 'iframe', size: {x: 875, y: 550}}">Ping</a></td>
